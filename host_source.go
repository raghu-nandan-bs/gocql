...
func (h *HostInfo) update(from *HostInfo) {
	...
	fmt.Printf("DEBUG:: Host update: hosts are unequal; proceeding with updates : %v, %v", h, from)
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	// not sure what about means, updating anyways
	h.peer = from.peer
...
func (r *ringDescriber) refreshRing() error {
	...
	for _, h := range hosts {
		...
		if host, ok := r.session.ring.addHostIfMissing(h); !ok {
			...
			r.session.logger.Printf("gocql: host before updating:: %v\n", host)
			host.update(h)
			r.session.logger.Printf("gocql: host after updating: %v\n", host)
			if host.connectAddress.String() != h.connectAddress.String() {
				...
				r.session.pool.removeHost(host.hostId)
